...
"""Ansible module for managing DataRobot use_case resources."""
...
---
module: use_case
short_description: Manage DataRobot use cases
description:
    - Create, update, and delete DataRobot use_case resources.
version_added: "1.0.0"
author:
...
options:
    ...
    description:
        description:
            - The description for the DataRobot resource.
        type: str
    state:
        description:
...
EXAMPLES = r"""
- name: Create a use_case
  stevefulme1.datarobot.use_case:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    name: "example"
    state: present

- name: Delete a use_case
  stevefulme1.datarobot.use_case:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    name: "example"
...
RETURN = r"""
use_case:
    description: The use_case resource details.
...
def find_resource(client, params):
    ...
    try:
        result = client.get("/api/v1/use_cases")
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/use_cases", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("use_case_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v1/use_cases/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("use_case_id", ""))
    client.delete(f"/api/v1/use_cases/{resource_id}")
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, use_case=resource)
        return
    ...
    module.exit_json(changed=False, use_case=existing)
