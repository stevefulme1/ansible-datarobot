...
"""Ansible module for managing DataRobot feature_list resources."""
...
---
module: feature_list
short_description: Manage DataRobot feature lists
description:
    - Create, update, and delete DataRobot feature_list resources.
version_added: "1.0.0"
author:
...
options:
    ...
    features:
        description:
            - The features for the DataRobot resource.
        type: str
    state:
        description:
...
EXAMPLES = r"""
- name: Create a feature_list
  stevefulme1.datarobot.feature_list:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    project_id: "example"
    state: present

- name: Delete a feature_list
  stevefulme1.datarobot.feature_list:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    project_id: "example"
...
RETURN = r"""
feature_list:
    description: The feature_list resource details.
...
def create_resource(client, params):
    ...
    return client.post(f"/api/v2/projects/{params['project_id']}/featurelists/", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("feature_list_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v2/projects/{params['project_id']}/featurelists/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("feature_list_id", ""))
    client.delete(f"/api/v2/projects/{existing.get('projectId', '')}/featurelists/{resource_id}")
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, feature_list=resource)
        return
    ...
    module.exit_json(changed=False, feature_list=existing)
