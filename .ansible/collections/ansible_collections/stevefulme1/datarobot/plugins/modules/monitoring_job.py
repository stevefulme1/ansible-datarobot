...
"""Ansible module for managing DataRobot monitoring_job resources."""
...
DOCUMENTATION = r"""
---
module: monitoring_job
short_description: Manage DataRobot monitoring jobs
description:
    - Create, update, and delete DataRobot monitoring_job resources.
version_added: "1.0.0"
author:
...
options:
    ...
    schedule:
        description:
            - The schedule for the DataRobot resource.
        type: str
    type:
        description:
...
EXAMPLES = r"""
- name: Create a monitoring_job
  stevefulme1.datarobot.monitoring_job:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    deployment_id: "example"
    state: present

- name: Delete a monitoring_job
  stevefulme1.datarobot.monitoring_job:
    api_url: "https://app.datarobot.com"
    api_key: "my-api-key"
    deployment_id: "example"
...
RETURN = r"""
monitoring_job:
    description: The monitoring_job resource details.
...
def find_resource(client, params):
    ...
    try:
        result = client.get("/api/v2/batchMonitoring")
...
def create_resource(client, params):
    ...
    return client.post("/api/v2/batchMonitoring", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("monitoring_job_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v2/batchMonitoring/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("monitoring_job_id", ""))
    client.delete(f"/api/v2/batchMonitoring/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['schedule', 'type']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, monitoring_job=resource)
        return
    ...
    module.exit_json(changed=False, monitoring_job=existing)
