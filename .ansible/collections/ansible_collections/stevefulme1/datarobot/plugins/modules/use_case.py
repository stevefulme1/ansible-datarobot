...
def create_resource(client, params):
    ...
    return client.post("/api/v2/useCases", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v2/useCases/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("use_case_id", ""))
    client.delete(f"/api/v2/useCases/{resource_id}")
