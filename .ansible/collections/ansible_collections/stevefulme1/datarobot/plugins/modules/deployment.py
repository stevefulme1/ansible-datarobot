...
"""Ansible module for managing DataRobot deployment resources."""
...
---
module: deployment
short_description: Manage DataRobot deployments
description:
    - Create, update, and delete DataRobot deployment resources.
version_added: "1.0.0"
author:
...
options:
    model_id:
        description:
            - The model id for the DataRobot resource.
        type: str
        required: true
    label:
        description:
            - The label for the DataRobot resource.
        type: str
    description:
        description:
    ...
    default_prediction_server_id:
        description:
            - The default prediction server id for the DataRobot resource.
        type: str
    state:
        description:
...
- name: Create a deployment
  stevefulme1.datarobot.deployment:
...
def create_resource(client, params):
    ...
    return client.post("/api/v2/deployments", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v2/deployments/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("deployment_id", ""))
    client.delete(f"/api/v2/deployments/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['label', 'description', 'default_prediction_server_id']
...
def main():
    argument_spec = dict(
        ...
        default_prediction_server_id=dict(type="str"),
