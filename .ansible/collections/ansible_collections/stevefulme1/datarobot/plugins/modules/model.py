...
"""Ansible module for managing DataRobot model resources."""
...
---
module: model
short_description: Manage DataRobot models
description:
    - Create, update, and delete DataRobot model resources.
version_added: "1.0.0"
author:
...
options:
    ...
    model_type:
        description:
            - The model type for the DataRobot resource.
        type: str
    state:
        description:
...
def find_resource(client, params):
    ...
    try:
        result = client.get(f"/api/v2/projects/{params['project_id']}/models/")
...
def create_resource(client, params):
    ...
    return client.post(f"/api/v2/projects/{params['project_id']}/models/", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v2/projects/{params['project_id']}/models/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("model_id", ""))
    client.delete(f"/api/v2/projects/{existing.get('projectId', '')}/models/{resource_id}")
