...
def find_resource(client, params):
    ...
    try:
        result = client.get("/api/v2/externalDataSources")
...
def create_resource(client, params):
    ...
    return client.post("/api/v2/externalDataSources", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v2/externalDataSources/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("data_source_id", ""))
    client.delete(f"/api/v2/externalDataSources/{resource_id}")
