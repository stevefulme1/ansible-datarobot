...
"""Ansible module for managing DataRobot project resources."""
...
---
module: project
short_description: Manage DataRobot projects
description:
    - Create, update, and delete DataRobot project resources.
version_added: "1.0.0"
author:
...
options:
    ...
    target_variable:
        description:
            - The target variable for the DataRobot resource.
        type: str
    state:
        description:
...
def find_resource(client, params):
    ...
    try:
        result = client.get("/api/v2/projects")
...
def create_resource(client, params):
    ...
    return client.post("/api/v2/projects", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v2/projects/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("project_id", ""))
    client.delete(f"/api/v2/projects/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['description', 'target_variable']
