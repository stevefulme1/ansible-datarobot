...
def _validate_host(host: str) -> bool:
    ...
    if "://" in host:
        logger.error(
            ...
            "Provide only the hostname, e.g. 'app.datarobot.com'.",
            host,
        )
        return False
...
async def main(queue: asyncio.Queue, args: dict[str, Any]) -> None:
    """Poll DataRobot API for events and push to EDA queue."""
    host = args["host"]
    ...
    while True:
        try:
            ...
            logger.error("Error polling DataRobot events: %s", exc)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(_MockQueue(), {"host": "app.datarobot.com", "interval": "10"}))
